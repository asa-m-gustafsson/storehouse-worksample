...
export default class ApiConverter implements IApiConverter {
  public extractItemListsFromState = (
      ...
      return (
        event.type ===
          (location === LocationType.Home
            ? EventType.Storage
            : EventType.Retrieval) && event.plannedTime > new Date()
      );
    });
    ...
    locatedRawEvents.forEach((event) => {
      ...
      rawItemsForEvent.forEach((rawItem) => {
        if (rawItem.group_id) {
          ...
          let groupEntryForRawItem: GroupType = entriesForEvent.find(
            (entry) => {
              return (
                !ListEntryIsItem(entry) && entry.groupId === rawItem.group_id
              );
            }
          ) as GroupType;
